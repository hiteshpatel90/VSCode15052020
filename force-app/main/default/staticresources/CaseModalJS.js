(function ( $ ) {
    ...
    window.caseModal = function( options ) {
        ...
        var defaultOptions = {
            ...
            listCaseSurgicalProceduresMethod: "CS_ARCCExtension.retrieveCaseSurgicaProcedures",
        ...
        */
        ...
        function afterProcedureSelected(){
                        
            //  Get all the selected Procedures to refresh the Surgical Techniques
            var procedureIds = [];
            $("#case-prop-modal div.procedures").children().each(function() {
                procedureIds.push($(this).attr("data-record-id"));
            });
            
            console.log("Selected Procedures: " + JSON.stringify(procedureIds));
            
            $("#case-prop-modal i.surgical-technique-busy").show();
            
            Visualforce.remoting.Manager.invokeAction('CS_ARCCExtension.searchSurgicalTechniques', procedureIds.join("::"),
            function(result, event) {
                ...
                settings.surgicalTechnique = surgicalTechs;
                ...
                var surgeTypeAheadOptions = {
                    minLength: 0,
                    items: 50,
                    source: settings.surgicalTechnique,
                    ...
                    afterSelect: function(item) {
                        ...
                        if ( $("#case-prop-modal span[data-record-id="+item.Id+"].ctag").length === 0 ) {
                            // no duplicates
                            $("#case-prop-modal div.ctag-list.surgical-technique").append(obj2ctag(item));
                        }
                        $("#case-prop-modal .surgical-technique-typeahead").val("");

                        afterAnyProcedureSelected();
                ...
                // init the type ahead
                $("#case-prop-modal input.surgical-technique-typeahead").typeahead('destroy','NoCached');
                $("#case-prop-modal input.surgical-technique-typeahead").typeahead(surgeTypeAheadOptions);

                $("#case-prop-modal i.surgical-technique-busy").hide();
            }); //  end of remote call
        }   //  end of afterProcedureSelected

        function afterAnyProcedureSelected(){

            //  Get all the selected Procedures to refresh the Surgical Techniques
            var procedureIds = [];
            $("#case-prop-modal div.surgical-technique").children().each(function() {
                procedureIds.push($(this).attr("data-record-id"));
            });
            
            console.log("Selected Surgical Technique: " + JSON.stringify(procedureIds));
            
            $("#case-prop-modal i.products-busy").show();
            
            Visualforce.remoting.Manager.invokeAction('CS_ARCCExtension.productsForSelectedProcedures', procedureIds.join("::"),
            function(result, event) {
            
                console.log("Products for Selected Procedures: " + JSON.stringify(result));
        ...
        }   //  end of afterAnyProcedureSelected
            ...
            for ( var i=0; i < settings.prefCardList.length; i++ ) {
                ...
                if ( item.Id === $(this).val() ) {
                    ...
                    if ( $("#case-prop-modal span[data-record-id="+item.Id+"].ctag").length === 0) {
                        ...
                        //  Set the Surgicaltechniques
                        setSurgicalTechniques('case-prop-modal', true);

                        //  Also Set the surgical technique from the pref card in modal
                        $("#case-prop-modal div.ctag-list.surgical-technique").append(obj2ctag({
                            Name: item.Surgical_Technique__r.Name,
                            Id: item.Surgical_Technique__c
                        }));
        ...
        $("#case-prop-modal input.procedure-typeahead").typeahead({
            ...
            items: 50,
            source: function(query, onSuccess) {
                search(query, settings.searchProceduresMethod, onSuccess); //  onSuccessProcedureLoad
            },
            displayText: function(item) {
                return decodeEntities(item.Name);
            },
            afterSelect: function(item) {                
                if ( $("#case-prop-modal span[data-record-id="+item.Id+"].ctag").length === 0 ) {
                    ...
                    setProductsFor(item, true, 'case-prop-modal');
                    afterProcedureSelected();
                }
        ...
        $("#case-prop-modal button.case-save-btn").on("click", function() {
            ...
            var surgicalIds = [];
            $("#case-prop-modal div.surgical-technique").children().each(function() {
                surgicalIds.push($(this).attr("data-record-id"));
            });
            ...
            var selectedOwnerId = $("#case-prop-modal .case-owner").val();
            ...
            //var localStartDate = moment(startDateStr, "MM/DD/YYYY h:mm A").tz(currenUserTZ);
            //var localEndDate = moment(endDateStr, "MM/DD/YYYY h:mm A").tz(currenUserTZ);
            ...
            console.log("start Date: " + localStartDate + "\t End Date: " + localEndDate + "\t Time Zone: " + currenUserTZ);

            //localStartDate = moment.tz(localStartDate, currenUserTZ);
            //localEndDate = moment.tz(localEndDate, currenUserTZ);
            ...
            Visualforce.remoting.Manager.invokeAction(settings.namespace+"CS_ARCCExtension.saveCase",
                ...
                participantIds, status, procedureIds, surgicalIds, selectedOwnerId,
                function(result, event) {
                    ...
                    if (event.status && settings.calendar) {
                        ...
                        //result.color = "#427cbb";
                        var userRow = $("table.calendar-users tbody tr[data-user-id="+result.ownerId+"]");
                        ...
                        /// if the owner is checked add to the calendar and show on the map
                        if ( $(userRow).find("input[type=checkbox]").is(":checked") ) { 
                            
                            $(""+settings.calendar).fullCalendar( 'renderEvent', result, 'stick');
                            result.mapShow();
                        ...
                        if ( result.participants.length > 0 ) {
                            for ( var k=0; k < result.participants.length; k++) {
						        ...
						        childEvent.providerTerritory = result.participants[k].participantTerritory;
                                childEvent.providerTerritoryId = result.participants[k].participantTerritoryId;
                                childEvent.providerParentTerritoryId = result.participants[k].participantParentTerritoryId;
                                childEvent.ownerProviderTerritory = result.participants[k].participantTerritory;
                                childEvent.ownerProviderTerritoryId = result.participants[k].participantTerritoryId;
                                childEvent.ownerProviderParentTerritoryId = result.participants[k].participantParentTerritoryId;
						        ...
						        var color = $("table.calendar-users tbody tr[data-user-id="+childEvent.ownerId+"]").attr("data-user-color");
                                if ( !color ) {
                                    // user doesn't exist, get the next color
                                    color = colors[ colorIdx++ ];
                                    $("table.calendar-users tbody tr[data-user-id="+childEvent.ownerId+"]").attr("data-user-color", color);
                                }
						        childEvent.color = color;
                                ...
                                if ( ! $(userRow).find("input[type=checkbox]").is(":checked") ) { 
                                    ...
                                    $(""+settings.calendar).fullCalendar( 'renderEvent', childEvent, 'stick');
                                    childEvent.mapShow();
                                }
        ...
        $("#case-prop-modal").on("shown.bs.modal", function (e) {
            ...
            var caseId = $("#case-prop-modal").attr("data-record-id");
            var currentUser = $("#case-prop-modal").attr("data-current-user");

            $('#case-prop-modal .case-owner').removeProp('disabled');

            $("#case-prop-modal .btn-clear-products").on("click", function(e) {
                e.preventDefault();
                e.stopPropagation();
                // deletes products selected in preference card
                $("#case-prop-modal div.products").empty();                     
            });

            if( ! caseId && currentUser){
                    //  if the event is due to Add Event click select Current Owner by default
                populateOwnerInfo(currentUser);
            }
            ...
            if ( caseId ) {
                ...
                // resolve procedures
                $("#case-prop-modal i.surgical-technique-busy").show();
                Visualforce.remoting.Manager.invokeAction(settings.listCaseSurgicalProceduresMethod,
                caseId,
                function(result, event) {   
