({
	itemSelected : function(component, event, helper) {
        ...
        if(SelIndex){
            var serverResult = component.get("v.server_result_con");
            var selItem = serverResult[SelIndex];
            if(selItem.val){
               component.set("v.selItem_con",selItem);
               component.set("v.last_ServerResult",serverResult);
            } 
            component.set("v.server_result_con",null); 
        } 
	}, 
    itemSelectedParent : function(component, event, helper, selInd) {
        
        var target = event.target;
        var SelIndex = selInd;
        
        if(selInd > 1){
            SelIndex = helper.getIndexFrmParent(target,helper,"data-selectedIndex");
        }
    ...
    serverCall : function(component, event, helper) {  
        ...
        if (event.keyCode == 27 || !searchText.trim()) { 
            ...
            component.set("v.server_result_con",component.get("v.last_ServerResult"));
    ...
    handleResponse : function (res,component,helper){
        if (res.getState() === 'SUCCESS') {
            ...
            if(retObj.length <= 0){
                ...
                component.set("v.server_result_con",noResult); 
            	component.set("v.last_ServerResult",noResult);
            }else{
                component.set("v.server_result_con",retObj); 
    ...
    clearSelection: function(component, event, helper){
        component.set("v.selItem_con",null);
        component.set("v.server_result_con",null);
    } 
})