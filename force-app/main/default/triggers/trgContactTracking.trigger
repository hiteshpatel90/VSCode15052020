trigger trgContactTracking on Contact (after update) { 
Field_History_Tracking_Admin_Setting__c ObjRecord = Field_History_Tracking_Admin_Setting__c.getInstance('Contact');
if(ObjRecord != null && ObjRecord.Enable_for_Field_Tracking__c == true){
if(helperContactTracking.isFieldTrackingTriggerExecuted == false){
    helperContactTracking.isFieldTrackingTriggerExecuted = true;
    List<Field_History__c> FieldHistoryUpdate = new List<Field_History__c>();
    List<Field_History_Tracking_Admin__c> FHTList = [select id, Fields_For_Tracking__c, sObject_Name__c from Field_History_Tracking_Admin__c where Enable_for_Field_Tracking__c = true and sObject_Name__c = 'Contact'];
    if(!FHTList.isEmpty()){
        ...
        if(strFieldTracking != null){
            ...
            for(Contact ab: trigger.new){
                for(String strField: arrFieldTracking){
                    ...
                    if(Schema.getGlobalDescribe().get('Contact').getDescribe().fields.getMap().keySet().contains(strField)){
